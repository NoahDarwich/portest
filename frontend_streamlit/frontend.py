...
file_path = "./frontend/data/full_df.csv"
...
with st.sidebar:
    ...
    Violence_Protesters = str(st.selectbox("Level of Violence of Protesters", violence_lst))
    Number_of_Participants = st.number_input("Number of Participants", value=3, step=1, min_value=3)
...
repression_predictions = response["predict0"].replace("[[", "").replace("]]", "").split()
...
rep_lst = [
    no_nown_coercion,
    arrests_and_detentions_probability,
    physical_harassment_probability,
    injuries_inflicted_probability,
    deaths_inflicted_probability,
    security_forces_present_at_event_probability,
    party_or_militias_present_at_event_probability,
]

methods_lst = {
    "teargas": teargas_probability,
    "rubber_bullets": rubber_bullets_probability,
...
def plot_map(df):
    df["violence_heat"] = np.where(
        df["repression"] != "No known coercion, no security presence", 1, 0
    )
    df.dropna(subset=["gpslatend", "gpslongend"])
    fig_new = px.density_mapbox(
        df,
        lat="gpslatend",
        lon="gpslongend",
        opacity=0.6,
        ...
        radius=10,
        center={"lat": 30.5852, "lon": 36.2384},
        zoom=4,
...
def propa_vs_labels(probas):
    ...
    keys_lst = list(propa_dict)
    for count, lst in enumerate(range(1, len(probas))):
